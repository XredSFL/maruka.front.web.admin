...
function AddGroupCompanyModal({ onClose, onSave }) {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
        <form onSubmit={handleSubmit} className="flex overflow-hidden flex-col justify-center max-w-2xl bg-white rounded-2xl p-8" role="dialog" aria-labelledby="modalTitle">
          <div className="flex flex-col items-center self-center pb-8 w-full max-md:max-w-full">
            <div className="flex overflow-hidden flex-col w-full text-3xl font-bold tracking-tight bg-white text-neutral-950 max-md:max-w-full">
              <div className="flex flex-wrap gap-5 justify-between w-full max-w-[624px] max-md:mr-2.5 max-md:max-w-full">
                <h1 id="modalTitle">Add News</h1>
            ...
            <div className="flex flex-col gap-4 mt-6 max-w-full w-[624px]">
              ...
              <div className="flex flex-col">
                <label htmlFor="News" className="text-sm font-medium tracking-normal leading-none text-black mb-1">
                  News (*)
                </label>
                <textarea
                  id="news"
                  ...
                  className="px-4 py-3 w-full text-base tracking-normal bg-white rounded-lg border border-solid border-stone-300 min-h-[150px] text-neutral-500"
                  required
                ></textarea>
              </div>
            </div>
...
function EditNewsModal({ onClose, onSave, item }) {
    const [formData, setFormData] = useState({
        title: '',
        news: '',
        photo: null
    ...
    const handleSubmit = async (e) => {
        ...
        for (const key in formData) {
            if (key === 'photo' && formData[key] === null) {
                continue; // Don't appssend photo if it hasn't changed
            }
            formDataToSend.append(key, formData[key]);
        }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
            <form onSubmit={handleSubmit} className="flex overflow-hidden flex-col justify-center max-w-2xl bg-white rounded-2xl p-8" role="dialog" aria-labelledby="modalTitle">
                <div className="flex flex-col items-center self-center pb-8 w-full max-md:max-w-full">
                    ...
                    <div className="flex flex-col gap-4 mt-6 max-w-full w-[624px]">
                        <div className="flex flex-col">
                            <label htmlFor="title" className="text-sm font-medium tracking-normal leading-none text-black mb-1">
                                Title (*)
                            </label>
                            <input
                                type="text"
                        ...
                        <div className="flex flex-col">
                            ...
                            {item && item.photo && (
                                <img src={item.photo} alt="Current news photo" className="mt-2 max-w-xs mb-2" />
                            )}
                            <input
                                type="file"
                                id="photo"
  ...
  function TableRow({ item }) {
    ...
    return (
      <div className="flex flex-wrap w-full bg-white px-8 border-b border-solid border-b-zinc-100 min-h-[70px] w-full">
        <div className="flex-1 shrink py-4 h-full text-sm tracking-normal min-w-[240px]" onClick={() => setIsModalOpen(true)}>
          <p style={{color:'#357049'}}>{item.title}</p>
        </div>
        ...
        <p style={{color:'#357049'}}>{item.createdAt}</p>
        </div>
        <div className="flex-1 shrink px-3 py-4 h-full text-sm tracking-normal whitespace-nowrap min-w-[240px]" onClick={() => setIsModalOpen(true)}>
          {item.photo ? (
            <img 
        ...
        <GroupCompanyDetailsModal
          companyData={{ id: item.id, title: item.title, photo: item.photo, news: item.news,  createdAt: item.createdAt, updatedAt: item.updatedAt }}
      ...
      <CategoryActionMenu
        ...
        />
          {isAddModalOpen && (
          <EditNewsModal onClose={() => setIsAddModalOpen(false)} onSave={handleSave} item={item}/>
            )}
      </div>
    );
  }
    ...
    <div className="flex flex-col shadow-[8px_8px_48px_rgba(0,0,0,0.08)] w-full">
      ...
      <div className="flex flex-wrap justify-between items-center px-3 mx-4 max-md:mr-2.5 max-md:max-w-full">
        ...
        <div className="flex flex-wrap flex-1 shrink justify-end gap-4 items-center self-stretch my-auto basis-0 min-w-[240px] max-md:max-w-full">
          ...
          <div className="flex flex-wrap w-full bg-white border-b px-8 border-solid border-b-zinc-100 min-h-[70px] w-full">
        <div className="flex-1 shrink py-4 h-full text-sm tracking-normal min-w-[240px] text-zinc-400">
          Title
        </div>
